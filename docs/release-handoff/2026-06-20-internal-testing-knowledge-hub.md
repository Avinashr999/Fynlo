...
1. Open Google Play Console > Setup > App integrity.
2. Copy the App signing key certificate SHA-1 and SHA-256, not just the upload key.
3. Open Firebase Console > Project settings > Android app `app.fynlo`.
4. Add both SHA fingerprints.
5. Ensure Google sign-in is enabled under Firebase Authentication > Sign-in method.
6. Download a fresh `google-services.json` if Firebase regenerates clients, and replace `app/google-services.json`.
7. Rebuild and upload a new internal testing AAB if the config file changed.
