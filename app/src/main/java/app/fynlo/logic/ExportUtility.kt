...
object ExportUtility {
    ...
    fun generatePDF(
        ...
        if (borrowers.isNotEmpty()) {
            ...
            val lw = listOf(usable*.22f, usable*.15f, usable*.10f, usable*.13f, usable*.12f, usable*.12f, usable*.16f)
            b.drawTableRow(listOf("Person","Principal","Rate","Lent On","Due","Status","Notes"), lw, isHeader = true)
            borrowers.forEachIndexed { i, bo ->
                val statusColor = if (bo.status == "Overdue") COLOR_RED else COLOR_BLACK
                b.drawTableRow(
                    listOf(bo.name, fmt(bo.amount), "${bo.rate}%", bo.date, bo.due.ifBlank{"-"}, bo.status, bo.notes),
                    lw, altBg = i % 2 == 0,
                    colors = listOf(COLOR_BLACK, COLOR_BLACK, COLOR_BLACK, COLOR_BLACK, COLOR_BLACK, statusColor, COLOR_GRAY)
                )
            }
        }
        ...
        b.canvas().drawText("Generated by Fynlo v3.1 | ${LocalDate.now()}", PAGE_W / 2f, b.y, footerPaint)
