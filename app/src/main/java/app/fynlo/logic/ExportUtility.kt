...
object ExportUtility {
    ...
    fun generatePDF(
        ...
        document.add(Paragraph("\n"))
        document.add(
            Paragraph("Generated by Fynlo v2.2 | ${LocalDate.now()}")
    ...
    fun generateMoneyFlowCSV(flows: List<app.fynlo.ui.screens.FlowEntry>): String {
        val sb = StringBuilder()
        ...
        // Summary
        val totalIn  = flows.filter { it.flowType == app.fynlo.ui.screens.FlowType.INCOME || it.flowType == app.fynlo.ui.screens.FlowType.DEBT_RECEIVED }.sumOf { it.amount }
        val totalOut = flows.filter { it.flowType == app.fynlo.ui.screens.FlowType.EXPENSE || it.flowType == app.fynlo.ui.screens.FlowType.DEBT_REPAY }.sumOf { it.amount }
        val lentOut  = flows.filter { it.flowType == app.fynlo.ui.screens.FlowType.LENDING }.sumOf { it.amount }
        sb.append("\n--- SUMMARY ---\n")
    ...
    fun generateMoneyFlowPDF(outputStream: OutputStream, flows: List<app.fynlo.ui.screens.FlowEntry>) {
        val writer   = PdfWriter(outputStream)
        val pdf      = PdfDocument(writer)
        val document = Document(pdf)

        document.add(Paragraph("Fynlo").setBold().setFontSize(24f).setFontColor(PRIMARY))
        ...
        flows.forEach { f ->
            val color = when (f.flowType) {
                app.fynlo.ui.screens.FlowType.INCOME        -> GREEN
                app.fynlo.ui.screens.FlowType.EXPENSE       -> RED
                app.fynlo.ui.screens.FlowType.DEBT_REPAY    -> RED
                else -> PRIMARY
            }
        ...
        document.add(tbl)
        document.add(Paragraph("\nGenerated by Fynlo | ${LocalDate.now()}").setFontSize(9f).setFontColor(ColorConstants.GRAY).setTextAlignment(TextAlignment.CENTER))
        document.close()
    }
    ...
    fun generateLoanStatementPDF(
        ...
        repayments: List<app.fynlo.data.model.Transaction>,
        interest: Double,
        outstanding: Double
    ) {
        val writer   = PdfWriter(outputStream)
        ...
        document.add(Paragraph("Fynlo | ${LocalDate.now()}").setFontSize(10f).setFontColor(ColorConstants.GRAY).setTextAlignment(TextAlignment.CENTER))
        document.add(Paragraph("\n"))
